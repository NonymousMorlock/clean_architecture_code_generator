...
class CubitGenerator extends GeneratorForAnnotation<CubitGenAnnotation> {
  ...
  void _generateCubit(StringBuffer buffer, RepoVisitor visitor) {
    ...
    final featureName =
        className.replaceAll('CubitTBG', '').replaceAll('Cubit', '');
  ...
  void _generateCubitMethod(
    ...
    final successState =
        _generateSuccessState(methodName, returnType, featureName);
    if (returnType.toLowerCase().trim() == 'void') {
      buffer.writeln('      (_) => emit(const $successState()),');
    } else {
      buffer.writeln(
        '      (${returnType.toLowerCase().startsWith('list') ? 'data' : returnType.camelCase}) => emit($successState(${returnType.toLowerCase().startsWith('list') ? 'data' : returnType.camelCase})),',
      );
    }
  ...
  void _generateStates(StringBuffer buffer, RepoVisitor visitor) {
    ...
    buffer.writeln(
      '  const ${featureName}Error({required this.message, required this.title});',
    );
    buffer.writeln();
    ...
    buffer.writeln(
      'final class Paginated${featureName}sFetchError extends ${featureName}Error {',
    );
  ...
  void _generateSuccessStateClass(StringBuffer buffer, String stateName,
      String returnType, String baseStateName) {
