...
class InjectionGenerator extends GeneratorForAnnotation<InjectionGenAnnotation> {
  @override
  ...
  void _generateInjectionContainerBase(StringBuffer buffer) {
    ...
    buffer.writeln('// This file contains all imports for the injection container');
    buffer.writeln('// It is automatically updated when new dependencies are added');
    buffer.writeln();
    ...
    buffer.writeln('// These imports will be automatically managed by the generator');
    buffer.writeln();
    buffer.writeln('part \'injection_container.main.dart\';');
    buffer.writeln();
  }

  void _generateInjectionContainerMain(StringBuffer buffer, RepoVisitor visitor) {
    final className = visitor.className;
    final featureName = className.replaceAll('RepoTBG', '').replaceAll('Repo', '');
  ...
  void _generateFeatureInit(StringBuffer buffer, RepoVisitor visitor, String featureName) {
    final className = visitor.className;
    ...
    buffer.writeln('    ..registerLazySingleton<$repoName>(() => $repoImplName(sl(), sl()))');
  ...
  void _generateServicesInit(StringBuffer buffer) {
    ...
    buffer.writeln('  final sharedPreferences = await SharedPreferences.getInstance();');
    ...
    buffer.writeln('    LogInterceptor(requestBody: true, responseBody: true),');
    buffer.writeln('    // TODO: Add additional interceptors as needed');
    buffer.writeln('    // RefreshTokenInterceptor(dio: dio, sessionBloc: sl<SessionBloc>()),');
    buffer.writeln('  ]);');
    buffer.writeln('  ');
